...
from agents.strategy_performance_expert import strategy_performance_expert, strategy_performance_expert_stream
from agents.comprehensive_holdings_analyst import comprehensive_holdings_analyst, comprehensive_holdings_analyst_stream
...
class PortfolioManagerAgent:
    def __init__(self, load_tools_from_directory=False):
        """初始化投资组合管理Agent"""
        ...
        self.agent = Agent(
            ...
            """,
            tools=[
                mem0_memory, current_time, retrieve, 
                strategy_performance_expert, comprehensive_holdings_analyst,
                portfolio_allocation_expert, market_trend_expert,
                manager_analyst, fees_analyst, user_profile_agent, fund_selector_agent
            ],
        ...
        self.stream_agent = Agent(
            ...
            - mem0_memory: 存储用户的投资偏好和历史交互信息
            - current_time: 获取当前时间
            - strategy_performance_expert_stream: 流式版本的基金策略与业绩专家，负责分析基金的投资策略、风格以及历史业绩、波动性和风险调整收益。
            - comprehensive_holdings_analyst_stream: 流式版本的综合持仓分析师，负责分析基金持仓股票表现，判断基金真实盈利能力。
            - portfolio_allocation_expert: 投资组合配置专家，负责分析用户现有投资组合，提供持有或调整建议。
            - market_trend_expert: 市场趋势专家，提供市场趋势分析和行业前景预测。
            - manager_analyst: 基金经理分析师，评估基金经理的管理能力和团队稳定性。
            - fees_analyst: 费用分析师，分析基金的费用结构和成本效益。
            ...
            tools=[
                mem0_memory, current_time, retrieve, 
                strategy_performance_expert_stream, comprehensive_holdings_analyst_stream,
                portfolio_allocation_expert, market_trend_expert,
                manager_analyst, fees_analyst, user_profile_agent, fund_selector_agent
            ],
            load_tools_from_directory=load_tools_from_directory,
    ...
    async def process_query_stream(self, query: str) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        """
        logger.info(f"异步处理用户查询: {query}")
        
        # 使用stream_async方法获取异步迭代器
        agent_stream = self.stream_agent.stream_async(query)
